...
public class PersistentEntities implements Streamable<PersistentEntity<?, ? extends PersistentProperty<?>>> {
	...
	public PersistentEntity<?, ?> getEntityUltimatelyReferredToBy(PersistentProperty<?> property) {
		...
		if (associationTargetType == null) {
			return getEntitiesIdentifiedBy(propertyType);
		}

		return getPersistentEntity(associationTargetType).orElseGet(() -> getEntitiesIdentifiedBy(propertyType));
	}
	...
	@Nullable
	private PersistentEntity<?, ?> getEntitiesIdentifiedBy(TypeInformation<?> type) {
